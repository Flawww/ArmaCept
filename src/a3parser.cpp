...
bool a3parser::decrypt_message(NetworkMessageRaw* src, int to) {
    ...
    if (to == TO_SERVER) {
        ...
        aes_crypt_cbc(&m_client_dec, AES_DECRYPT, length, iv, src->m_buf, src->m_buf);
    }
    else {
        memcpy(iv, server_iv, 16);
        aes_crypt_cbc(&m_server_dec, AES_DECRYPT, length, iv, src->m_buf, src->m_buf);
    }
...
bool a3parser::encrypt_message(NetworkMessageRaw* src, int to) {
    ...
    if (to == TO_SERVER) {
        memcpy(iv, client_iv, 16);
        aes_crypt_cbc(&m_client_enc, AES_ENCRYPT, src->m_pos, iv, src->m_buf, src->m_buf);
    }
    else {
        memcpy(iv, server_iv, 16);
        aes_crypt_cbc(&m_server_enc, AES_ENCRYPT, src->m_pos, iv, src->m_buf, src->m_buf);
    }
